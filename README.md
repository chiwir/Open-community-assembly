...
The dataset was generated by varying the variance of Gaussian kennel for intrinsic rate, variance of Gaussian kennel for interaction strength and the mean $\lambda$ for generating number of species to be introduced using a Poisson distribution. For each combination the community dynamics shows three phase, **initial phase** before introduction: 
an exponetional decline of species richness, **transient phase** a linear increase in species richness, the **stationary phase** when the rate of extinction is approximately equal to the introduction rate.
A snapshot of 1000 time steps in the stationary phase was used to calculate the mean values for each variable in the dataset.
